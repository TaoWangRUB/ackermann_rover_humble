...
- [ ] 12.1 **Docker build:** Start container `./scripts/start_docker.sh`, then `cd /workspace && colcon build --packages-select rover_monitor` — verify clean build with ROS 2 message generation + Protobuf codegen
- [ ] 12.2 **Docker launch:** Inside container, `ros2 launch rover_monitor monitor.launch.py use_sim_time:=true` — verify component_container starts single-threaded, all 5 components load without errors
- [ ] 12.3 **Docker topics:** Verify topic publication via `ros2 topic list` and `ros2 topic echo`:
...
- [ ] 12.4 **Docker MQTT telemetry:** Start Mosquitto inside Docker, publish mock `/fmu/out/vehicle_global_position` (PX4 heartbeat), verify `mosquitto_sub -h localhost -t "rover/health/#"` receives Protobuf-serialized RoverHealth messages with correct field values
- [ ] 12.5 **Docker alert triggering:** Publish degraded telemetry (e.g., low battery via `/fmu/out/battery_status` with `remaining=15.0`), verify PX4_BATTERY_CRITICAL alert appears in `/monitor/health` message and `rover/alerts` MQTT topic
...
- [ ] 13.1 Add `RoverCommand`, `CommandAck`, `NavGoal`, `SetMode`, `SetParam` messages to `proto/rover_health.proto`
- [ ] 13.2 Add `CommandType` enum (CMD_UNKNOWN, CMD_NAV_GOAL, CMD_ARM, CMD_DISARM, CMD_SET_MODE, CMD_ESTOP, CMD_CANCEL_GOAL, CMD_SET_PARAM) and `AckStatus` enum (ACK_RECEIVED, ACK_ACCEPTED, ACK_REJECTED, ACK_COMPLETED, ACK_FAILED)
- [ ] 13.3 **Docker verification:** Inside rover container, build with extended schema, verify:
...
- [ ] 14.1 Add MQTT inbound subscriptions to telemetry_publisher component (`rover/cmd/goal`, `rover/cmd/arm`, `rover/cmd/mode`, `rover/cmd/estop`) with QoS 2, sharing the existing `paho::async_client`
- [ ] 14.2 Implement Protobuf `RoverCommand` deserialization; log and drop malformed payloads
- [ ] 14.3 Implement cmd_id deduplication — bounded `std::unordered_set<std::string>` with 30s eviction to guard against MQTT QoS 2 re-delivery
- [ ] 14.4 Implement command dispatch table: CMD_NAV_GOAL → `NavigateToPose` action, CMD_ARM/CMD_DISARM → `/fmu/in/vehicle_command`, CMD_SET_MODE → `/fmu/in/vehicle_command`, CMD_ESTOP → zero `TwistStamped` + disarm, CMD_CANCEL_GOAL → cancel active Nav2 goal
- [ ] 14.5 Implement `CommandAck` Protobuf serialization and MQTT publish to `rover/cmd/ack` (QoS 1) — ACK_RECEIVED immediately, then ACK_ACCEPTED/ACK_REJECTED/ACK_COMPLETED/ACK_FAILED after ROS 2 dispatch
...
- [ ] 15.1 Create `control_center/` directory with `main.py`, `cc/` module (mqtt_receiver.py, telemetry_cache.py, influxdb_writer.py, alert_notifier.py, dashboard_server.py, cmd_gateway.py, safety_gate.py, ack_tracker.py, event_bus.py), `proto/`, `ui/src/`, `requirements.txt`
- [ ] 15.2 Create `config/control_center.yaml` — master config with mqtt, telemetry_cache, dashboard, influxdb, cmd_gateway, and notifier sections
- [ ] 15.3 Create `config/notifier.yaml` — notification channel config (terminal, sound, desktop, Slack, email) with per-channel enabled/min_severity/settings

## 16. CC-1: MQTT Receiver

- [ ] 16.1 Implement `cc/mqtt_receiver.py` — paho-mqtt async client subscribing to `rover/health/*`, `rover/alerts`, `rover/cmd/ack` with appropriate QoS
- [ ] 16.2 Implement Protobuf deserialization dispatching typed events (`evt.health`, `evt.health.cam`, `evt.health.px4`, `evt.health.jetson`, `evt.alert`, `evt.cmd_ack`) via `loop.call_soon_threadsafe`
- [ ] 16.3 Implement reconnection with exponential backoff (min 1s, max 30s); re-subscribe on `on_connect`; emit `evt.connection_lost` / `evt.connection_restored`

## 17. CC-2: Telemetry Cache

- [ ] 17.1 Implement `cc/telemetry_cache.py` — `asyncio.Lock`-protected dict keyed by subsystem (health, cam, px4, jetson) with `updated_at` monotonic timestamp
- [ ] 17.2 Implement staleness detection: if `time.monotonic() - updated_at > stale_threshold_s` (default 5s), report stale; safety gate treats stale cache as ERROR

## 18. CC-3: InfluxDB Writer

- [ ] 18.1 Implement `cc/influxdb_writer.py` — async batch writes for `rover_telemetry` measurement (batch_size=50, flush_interval=2000ms, max_retries=3)
- [ ] 18.2 Implement sync immediate writes for `rover_alerts` and `rover_commands` measurements (cmd_id, cmd_type, issued_by, ack_status, round_trip_ms)
- [ ] 18.3 Implement connection failure handling — log errors on InfluxDB outage, continue processing events; live dashboard via WebSocket unaffected

## 19. CC-4: Alert Notifier

- [ ] 19.1 Implement `cc/alert_notifier.py` — alert deduplication with per-alert_id cooldown (default 60s); suppress re-notification within window
- [ ] 19.2 Implement notification channels: terminal (formatted log), sound (WAV playback), desktop (plyer cross-platform notification)
- [ ] 19.3 Implement notification channels: Slack (webhook POST with @mention), email (SMTP with configurable recipients) — both disabled by default

## 20. CC-5: Dashboard Server

- [ ] 20.1 Implement `cc/dashboard_server.py` — FastAPI REST endpoints: `POST /api/command`, `GET /api/command/history`, `GET /api/health/history`, `GET /api/status`
- [ ] 20.2 Implement WebSocket endpoints: `/ws/health` (2 Hz telemetry push from cache), `/ws/alerts` (alert events), `/ws/cmd_ack` (command ACK events)
- [ ] 20.3 Build React SPA in `ui/src/` — panels: connection status, overall health, battery, camera, PX4, Jetson, SLAM latency, active alerts, command panel (goal/arm/disarm/mode/e-stop), command log
- [ ] 20.4 Configure multi-stage Dockerfile: first stage builds React via `npm run build`, second stage copies `dist/` into Python runtime image

## 21. CC-6: Command Gateway

- [ ] 21.1 Implement `cc/safety_gate.py` — per-CommandType precondition checks reading telemetry_cache: CMD_NAV_GOAL requires armed+offboard+slam<200ms+not-error+not-stale; CMD_ARM requires connected+battery>20%+not-stale; CMD_ESTOP always passes
- [ ] 21.2 Implement `cc/cmd_gateway.py` — assign UUID v4 cmd_id, serialize `RoverCommand` to Protobuf
- [ ] 21.3 Implement MQTT publish to `rover/cmd/*` topics with QoS 2; log command to InfluxDB via cmd_logger immediately

## 22. CC-7: ACK Tracker

- [ ] 22.1 Implement `cc/ack_tracker.py` — dict of pending commands keyed by cmd_id with `asyncio.Future`; resolve future on matching `CommandAck`; update InfluxDB and broadcast to `/ws/cmd_ack`
- [ ] 22.2 Implement configurable `ack_timeout_s` (default 5s) — mark as ACK_FAILED with "ACK timeout" if no response; clean up pending entry

## 23. Host Deployment

- [ ] 23.1 Create `control_center/docker-compose.yaml` with services: mosquitto (eclipse-mosquitto:2, port 1883), influxdb (influxdb:2, port 8086, auto-init org/bucket/token), control_center (build from Dockerfile, port 8080, depends_on mosquitto+influxdb)
- [ ] 23.2 Create `config/mosquitto.conf` with listener 1883, persistence enabled, allow_anonymous true (local LAN)
- [ ] 23.3 **Docker e2e integration test:**
...
- [ ] 24.1 **Docker rover build:** Inside rover container (`./scripts/start_docker.sh`), run `colcon build --packages-select rover_monitor` — verify extended Protobuf schema (telemetry + RoverCommand + CommandAck messages) compiles without errors
- [ ] 24.2 **Host stack launch:** From host machine (outside rover container), run `docker compose -f control_center/docker-compose.yaml up -d` — verify services start:
...
- [ ] 24.3 **Network bridging verification:** Configure rover container to reach host MQTT broker (use `host.docker.internal:1883` on Docker Desktop, or docker-compose network bridge on Linux). Inside rover container, test: `mosquitto_sub -h host.docker.internal -t "rover/#"`
- [ ] 24.4 **Bidirectional telemetry flow:** Launch rover monitor inside rover container → publish telemetry to host Mosquitto → verify in InfluxDB dashboard at `localhost:8086` and control_center dashboard at `localhost:8080` (camera, PX4, Jetson panels updated at 2 Hz)
- [ ] 24.5 **Command flow rover→host:** Publish test command from control_center dashboard (e.g., arm request) → verify Protobuf command serialized to `rover/cmd/arm` MQTT topic → inside rover container, verify command received and ACK published to `rover/cmd/ack` → ACK visible in dashboard command log
- [ ] 24.6 **InfluxDB data validation:** Query InfluxDB (via `influx query` CLI or UI): verify `rover_telemetry` measurement contains camera/px4/jetson/slam_latency fields; verify `rover_alerts` contains alert_id/severity/timestamp; verify `rover_commands` contains cmd_id/cmd_type/issued_by/ack_status
