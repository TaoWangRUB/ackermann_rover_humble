...
3. Add `nav2_bringup.launch.py` that loads the config, starts the Nav2 lifecycle stack (controller/planner/bt_navigator/behavior_server/map_server/waypoint_follower), and maps outputs to `/cmd_vel_nav` (which enters the smoother yielding `/cmd_vel_smoothed`, then through the collision monitor yielding `/cmd_vel`); include args for `bt_xml`, `use_sim_time`, and config overrides.
