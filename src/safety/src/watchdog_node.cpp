...
public:
	SafetyWatchdogNode() : rclcpp::Node("safety_watchdog") {
		...
		pub_stop_ = this->create_publisher<ackermann_msgs::msg::AckermannDriveStamped>(cmd_topic_, 10);
		sub_fault_ = this->create_subscription<std_msgs::msg::Bool>(
				fault_topic_, 10,
				std::bind(&SafetyWatchdogNode::onFault, this, std::placeholders::_1));

		RCLCPP_INFO(get_logger(), "Safety watchdog started. Monitoring faults on %s", fault_topic_.c_str());
	}
