...
$DOCKER_COMPOSE --env-file .env -f docker/docker-compose.yml up -d ackermann_slam
sleep 5 # wait for container to be ready

echo -e "${GREEN}=== Running Build ===${NC}"
# Exclude submodule example and test packages from the build
$DOCKER_COMPOSE --env-file .env -f docker/docker-compose.yml exec ackermann_slam bash -c "
...
$DOCKER_COMPOSE --env-file .env -f docker/docker-compose.yml exec ackermann_slam bash -c "source /opt/ros/jazzy/setup.bash && colcon build --symlink-install --packages-up-to px4_ros2_cpp"
# Stage 2: Source stage-1 artifacts, then build the project's own packages
$DOCKER_COMPOSE --env-file .env -f docker/docker-compose.yml exec ackermann_slam bash -c "source /opt/ros/jazzy/setup.bash && source install/setup.bash && colcon build --symlink-install --packages-up-to ackermann_control safety px4_bringup robot_bringup ackermann_nav2_bringup rtabmap_bringup description_robot"

echo -e "${GREEN}=== Running Unit Tests ===${NC}"
# Only test project packages — skip submodule tests (px4-ros2-interface-lib examples, etc.)
$DOCKER_COMPOSE --env-file .env -f docker/docker-compose.yml exec ackermann_slam bash -c "source /opt/ros/jazzy/setup.bash && source install/setup.bash && colcon test --event-handlers console_direct+ --packages-select ackermann_control safety && colcon test-result --verbose"
...
$DOCKER_COMPOSE --env-file .env -f docker/docker-compose.yml exec ackermann_slam bash -c "source /opt/ros/jazzy/setup.bash && source install/setup.bash && timeout --preserve-status 25 ros2 launch robot_bringup robot_bringup.launch.py rtabmap:=true nav2:=true || true"

echo -e "${GREEN}=== Running Safety Checks & Linting ===${NC}"
$DOCKER_COMPOSE --env-file .env -f docker/docker-compose.yml exec ackermann_slam bash -c "source /opt/ros/jazzy/setup.bash && ament_lint_cmake . || true"
$DOCKER_COMPOSE --env-file .env -f docker/docker-compose.yml exec ackermann_slam bash -c "source /opt/ros/jazzy/setup.bash && ament_flake8 . || true"
$DOCKER_COMPOSE --env-file .env -f docker/docker-compose.yml exec ackermann_slam bash -c "source /opt/ros/jazzy/setup.bash && ament_cpplint . || true"
