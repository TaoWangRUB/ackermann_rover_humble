...
while (( SECONDS < DEADLINE )); do
    mode_count="$({ dcomp exec -T ackermann_slam bash -lc "source /opt/ros/jazzy/setup.bash && source /workspace/install/setup.bash && ros2 topic info /px4_modes/announce 2>/dev/null | awk '/Publisher count:/ { print \$3 }'"; } 2>/dev/null | tr -d '[:space:]')"
